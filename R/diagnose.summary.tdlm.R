...
diagnose.summary.tdlm <- function(x, ...) {
  ...
  # check object class
  if (class(x) != "summary.tdlm") {
    stop("The class of the model summary must be 'summary.tdlm'")
  }
  ...
  shinyApp(
    ui = fluidPage(
      ...
      navbarPage(
        ...
        tabPanel(
          "Model",
          fluidRow(column(6, plotOutput("dlm.trace")), 
                   column(6, plotOutput("ce.trace.plot"))),
          fluidRow(column(6, plotOutput("dlm.density")), 
        ...
        tabPanel(
          ...
          sidebarPanel(selectInput(
            "lag.choice",
            "Select lag:",
            choices = 1:ncol(mcmc.samples$dlm.mcmc)
          )),
          mainPanel(fluidRow(
    ...
    server = function(input, output, session) {
      ...
      ce <- unlist(mcmc.samples[["cumulative.mcmc"]])
      mcmc.obj <- coda::mcmc(ce)
      cum.mean <- mean(ce)
      cum.ci <- quantile(ce, probs = c(0.025, 0.975))
      
      # Model Fit Plot using ggplot2
      ...
      # *** Tab 3: Tree size ***
      tree.df <- as.data.frame(mcmc.samples$tree.size)
      ...
      # MH accept ratio
      tree.accept <- as.data.frame(mcmc.samples$accept) 
      ...
      output$accept.plot <- renderPlot({
        ...
        # Create a plot
        ggplot(tree.accept, aes(x = factor(step), fill = decision)) +
