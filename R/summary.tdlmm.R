...
summary.tdlmm <- function(x, conf.level = 0.95, marginalize = "mean", log10BF.crit = 0.5, mcmc = FALSE, verbose = TRUE, ...)
{
  ...
  } else if (length(marginalize) == ret$nExp & is.numeric(marginalize)) {
    ret$marg.values <- marginalize
  } else {
    stop("`marginalize` is incorrectly specified, see ?summary.tdlmm for details")
  }
  names(ret$marg.values) <- ret$expNames


  if(verbose){
    cat("Specified co-exposure values:\n")
    for(exp in 1:length(ret$expNames)){
      cat("-", ret$expNames[exp], ":", ret$marg.values[exp])
      cat("\n")
    }
    cat("\n")
  }
  ...
  ret$expSel        <- rep(FALSE, ret$nExp)
  names(ret$expSel) <- ret$expNames
  if (ret$mixPrior > 0) {
    # individual exposures
    priorProbInc <- 1 - exp(lgamma(2*ret$ctr$n.trees + ret$nExp*ret$mixPrior) +
                          lgamma((ret$nExp + 1)*ret$mixPrior) -
                          lgamma(2*ret$ctr$n.trees + (ret$nExp + 1)*ret$mixPrior) -
                          lgamma(ret$nExp*ret$mixPrior))
  ...
  for (i in 1:ret$nExp) {
    ...
    est <- dlmEst(as.matrix(x$TreeStructs[idx, -(3:4)]), ret$lags, mcmcIter)
    # est: dim 1 = time, dim 2 = iteration
    ret$DLM[[i]] <- list("mcmc" = est,
                         "marg" = array(0, dim(est)),
                         "name" = x$expNames[i])
  }
  names(ret$DLM)  <- ret$expNames
  ...
  for (i in sort(unique(x$MIX$exp1))) {
    for (j in sort(unique(x$MIX$exp2))) {
      if (verbose) {
        if (nMix == ceiling(0.25 * ret$nMix)) {cat("25%...")}
        if (nMix == ceiling(0.5 * ret$nMix)) {cat("50%...")}
        if (nMix == ceiling(0.75 * ret$nMix)) {cat("75%...")}
        if (nMix == ret$nMix) {cat("100%\n")}
        nMix <- nMix + 1
      }

      idx <- which(x$MIX$exp1 == i & x$MIX$exp2 == j)
      if (length(idx) > 0) {
        est <- mixEst(as.matrix(x$MIX[idx,,drop = FALSE]), ret$nlags, mcmcIter)
        m   <- paste0(x$expNames[i + 1], "-", x$expNames[j + 1])
        ret$MIX[[m]] <-
          list("matfit"   =  sapply(1:ret$nlags, function(k) rowMeans(est[,k,,drop=FALSE])),
               "cilower"  =  sapply(1:ret$nlags, function(k) {
                              apply(est[,k,], 1, quantile, probs = ret$ci.lims[1]) }),
               "ciupper"  =  sapply(1:ret$nlags, function(k) {
        ...
        ret$DLM[[i + 1]]$marg <- ret$DLM[[i + 1]]$marg +
          t(sapply(1:ret$nlags, function(k) colSums(est[k,,]))) *
        ...
        if (i == j) {
          est <- 0.5 * est * array(upper.tri(diag(ret$nlags), diag = TRUE), dim(est)) +
            0.5 * aperm(est, c(2, 1, 3)) *
            array(upper.tri(diag(ret$nlags), diag = TRUE), dim(est))
          
          if (mcmc) {
            ret$MIX[[m]]$mcmc   <- est
          }

          ret$MIX[[m]]$matfit   <- sapply(1:ret$nlags, function(k) {rowMeans(est[,k,,drop=FALSE]) })
          ret$MIX[[m]]$cilower  <- sapply(1:ret$nlags, function(k) {apply(est[,k,], 1,  quantile, probs = ret$ci.lims[1]) })
          ret$MIX[[m]]$ciupper  <- sapply(1:ret$nlags, function(k) {apply(est[,k,], 1, quantile, probs = ret$ci.lims[2]) })
        }

        ret$MIX[[m]]$cw <- (ret$MIX[[m]]$cilower > 0 | ret$MIX[[m]]$ciupper < 0)

        # Range of confidence levels for plots
        mixCIs <- lapply(1:ret$nlags, function(k) {apply(est[,k,], 1, quantile, probs = c(1:10/200, 190:199/200))})

        ciProbs <- c(99:90/100, 0)
        ret$MIX[[m]]$cw.plot <-
          sapply(1:ret$nlags, function(k) {
            ciProbs[
              sapply(1:ret$nlags, function(l) {
                min(c(11,which(
  ...
  # ---- Return ----
  ret$sig2noise <- ifelse(is.null(x$sigma2), NA,
