                              ...
                              double treevar, bool updateNested)
{
  ...
  for (s = 0; s < modTerm.size(); ++s) {
    nestedTerm.push_back(modTerm[s]->nodevals->nestedTree->listTerminal(updateNested));
  ...
  out.draw = ThetaHat;
  out.draw.noalias() += VThetaChol * as<Eigen::VectorXd>(rnorm(totTerm, 0.0, sqrt(ctr->sigma2)));
...
Rcpp::List dlmtreeTDLMNestedGaussian(const Rcpp::List model)
{
  ...
  for (ctr->b = 1; ctr->b <= (ctr->iter + ctr->burn); (ctr->b)++) {
    ...
    for (t = 0; t < ctr->nTrees; t++) {
      dlmtreeTDLMNestedGaussian_TreeMCMC(t, modTrees[t], expNS, ctr, dgn, Mod, Exp);
