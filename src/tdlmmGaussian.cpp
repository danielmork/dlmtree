...
void tdlmmGaussianTreeMCMC(int, Node*, Node*, tdlmCtr*, tdlmLog*,
...
treeMixMHR mixMHR(std::vector<Node*>, std::vector<Node*>, tdlmCtr*,
...
Rcpp::List tdlmmGaussian(const Rcpp::List model)
{
  ...
  for (ctr->b = 1; ctr->b <= (ctr->iter + ctr->burn); (ctr->b)++) {
    ...
    for (t = 0; t < ctr->nTrees; t++) {
      tdlmmGaussianTreeMCMC(t, trees1[t], trees2[t], ctr, dgn, Exp);
...
void tdlmmGaussianTreeMCMC(int t, Node *tree1, Node *tree2,
