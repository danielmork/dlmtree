...
void tdlnmGaussianTreeMCMC(int, Node*, tdlnmCtr*, tdlnmLog*, exposureDat*);
...
treeMHR dlnmMHR(std::vector<Node*>, tdlnmCtr*, Eigen::VectorXd, double, Node*, bool);
...
void tdlnmGaussianTreeMCMC(int t, Node *tree, tdlnmCtr *ctr, tdlnmLog *dgn, exposureDat *Exp)
{
  int i, step;
  int success = 0;
  double stepMhr = 0;
  ...
  // propose update
  stepMhr = tdlnmProposeTree(tree, Exp, ctr, step);
  ...
  // Rcout << (0.5 * mhr0.nTerm + 0.5) / (0.5 * mhr0.nTerm + 0.5 + 1.0 / (mhr0.termT2 /
  //   (2.0 * ctr->sigma2 * ctr->nu) + xiInv)) << "\n";
  ...
  // (ctr->Rmat).col(t) = mhr0.draw(0) * (dlnmTerm[0]->nodevals)->X;
  // for(i = 1; i < mhr0.nTerm; ++i)
  //   (ctr->Rmat).col(t) += mhr0.draw(i) * (dlnmTerm[i]->nodevals)->X;
...
treeMHR dlnmMHR(std::vector<Node*> nodes, tdlnmCtr *ctr,
                Eigen::VectorXd ZtR, double var, Node* tree, bool newTree)
{
  ...
  // pX > 1
    ...
    if (newTree) {
      tempV = out.Xd.transpose() * out.Xd - ZtX.transpose() * VgZtX;//tempV;
      out.tempV = tempV;
    } else {
