...
treeMHR monoDlnmMHR3(std::vector<Node*> dlnmTerm, tdlmCtr* ctr, VectorXd ZtR, double treevar, Node* tree, bool updateNested)
{
  treeMHR out;
  int totTerm = 0;
 ...
 */
void monoTDLNMTreeUpdate3(int t, Node* tree, tdlmCtr* ctr, tdlmLog* dgn, exposureDat* Exp, NodeStruct* nsX)
{
  int step =        0;
  int success =     0;
  double stepMhr =  0.0;
  ...
  if (tree->nodevals->tempV.rows() > 0)
    mhr0 = monoDlnmMHR3(dlnmTerm, ctr, ZtR, treevar, tree, 0);
  else
    mhr0 = monoDlnmMHR3(dlnmTerm, ctr, ZtR, treevar, tree, 1);
  ...
  // MHR
  mhr = monoDlnmMHR3(newDlnmTerm, ctr, ZtR, treevar, tree, 1);
  ...
  for (Node* eta : dlnmTerm) {
    ...
    // calculate MHR
    mhr = monoDlnmMHR3(dlnmTerm, ctr, ZtR, treevar, tree, 1);
...
} // end function monoTDLNMTreeUpdate3
...
Rcpp::List monotdlnm3_Cpp(const Rcpp::List model)
{
  ...
  for (ctr->b = 1; ctr->b <= (ctr->iter + ctr->burn); (ctr->b)++) {
    ...
    for (t = 0; t < ctr->nTrees; t++) {
      if (ctr->debug)
        Rcout << "\n" << ctr->b << ":" << t << ":";
      monoTDLNMTreeUpdate3(t, trees[t], ctr, dgn, Exp, nsX);
