...
void dlmtreeNested_TreeMCMC(int t, Node* modTree, 
                        NodeStruct* expNS, NodeStruct* modNS,
...
Rcpp::List dlmtreeNested(const Rcpp::List model)
{
  ...
  if (ctr->binomial)
    ...
    Exp = new exposureDat(as<MatrixXd>(model["Tcalc"]), ctr->Z, ctr->Vg);
  ctr->pX       = Exp->pX;
  ...
  for (ctr->b = 1; ctr->b <= (ctr->iter + ctr->burn); (ctr->b)++) {
    ...
    for (t = 0; t < ctr->nTrees; t++) {
      dlmtreeNested_TreeMCMC(t, modTrees[t], expNS, modNS, ctr, dgn, Mod, Exp);
                         ...
                         modDat* Mod, exposureDat* Exp)
{
  ...
  // Node* newTree = new Node(0, 1);
  // newTree->nodestruct = modNS->clone();
  // drawTree(newTree, newTree, ctr->treePriorMod[0], ctr->treePriorMod[1]);
  // newTree->setUpdate(1);
  // newTree->setUpdateXmat(1);
  ...
  for (Node* tn : modTerm) {
    // Node* newTree = new Node(0, 1);
    // newTree->nodestruct = expNS->clone();
    // drawTree(newTree, newTree, ctr->treePrior[0], ctr->treePrior[1]);
    // newTree->setUpdate(1);
    // newDlmTerm = newTree->listTerminal();
    // for (Node* nt : newDlmTerm) {
    //   Exp->updateNodeVals(nt);
    // }

    // tn->nodevals->updateXmat = 1;
    // tn->nodevals->nestedTree->proposed = newTree;

    // mhr   = dlmtreeNested_MHR(modTerm, ctr, ZtR, treevar, 1);
    // ratio = calcLogRatioTDLM(mhr0, mhr, RtR, RtZVgZtR, ctr, stepMhr, treevar);

    // tn->nodevals->nestedTree->proposed = 0;
    // if ((log(R::runif(0, 1)) < ratio) && (ratio == ratio)) {
    //   mhr0    = mhr;
    //   success = 2;
    //   tn->nodevals->nestedTree->replaceTree(newTree);
...
} // end dlmtreeNested_TreeMCMC function
                         ...
                         bool updateNested)
{
  ...
  for (s = 0; s < modTerm.size(); ++s) {
    ...
    // if ((modTerm[s]->nodevals->updateXmat) || ctr->binomial) {
      ...
      Otemp.resize(modTerm[s]->nodevals->idx.size()); Otemp.setOnes();
      ...
      modTerm[s]->nodevals->XtX       = Xtemp.transpose() * Otemp.asDiagonal() * Xtemp;
      modTerm[s]->nodevals->ZtXmat    = Ztemp.transpose() * Xtemp;
      modTerm[s]->nodevals->VgZtXmat  = ctr->Vg * modTerm[s]->nodevals->ZtXmat;
      XtR.segment(start, pX)          = Xtemp.transpose() * Otemp.asDiagonal() * Rtemp;
      // } else {
  ...
  // Rcout << ".\n";
  return(out);
} // End dlmtreeNested_MHR function